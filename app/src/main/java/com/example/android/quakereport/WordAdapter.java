...
//      call ContextCompat getColor() to convert the color resource ID into an actual integer color value,
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent){
        ...
        String newMagnitude = formatMagnitude(originalMagnitude);
        TextView magnitudeView = (TextView) listItemView.findViewById(R.id.magnitude);
        magnitudeView.setText(newMagnitude);
...
//        setting the location with primary and offset
        String originalLocation = currentWord.getCity();

        String primaryLocation;
        String locationOffset;
        if(originalLocation.contains(LOCATION_SEPARATOR)){
            String[] partsOfOriginal = originalLocation.split(LOCATION_SEPARATOR);

            locationOffset = partsOfOriginal[0] + LOCATION_SEPARATOR;
            primaryLocation = partsOfOriginal[1];
        }else{
